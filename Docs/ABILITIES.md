# ABILITY AND HEDIFF DESCRIPTIONS
...
### PHASE-LOCK (Thematic Concept)

Thematic: The total synchronization and capture of an unshielded signal loop. The female Cradlemold views the male target's reproductive clock not as an independent system, but as a stray frequency that must be bound to her own. To Phase-Lock a target is to freeze his cellular autonomy, forcing his internal biological rhythm into an absolute, inescapable timing alignment with her maternal engine.

Action: The user establishes a rigid harmonic bridge with a male target, temporarily hijacking his genetic sequence. This aggressive synchronization extracts the precise biological blueprints required to ignite her own reproductive cycle, securing a 90% pregnancy success rate through forced cellular compliance.

Narrative: Individual choice is silenced by the absolute precision of the signal. The Cradlemold does not negotiate for genetic material; it issues a command. She locks onto the target's frequency, overriding his system's defenses and drawing the necessary catalyst directly from his marrow. His biology is briefly transformed into an unyielding transmitter, forced to broadcast the exact markers her womb requires to shape the next generation of the hive.
